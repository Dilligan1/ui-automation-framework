...
**[Actions → UI tests → Run workflow](https://github.com/Dilligan1/ui-automation-framework/actions/workflows/tests.yml)**

Кнопка открывает форму: выбрать набор тестов (`smoke`, `regress`, `e2e`, `critical` или всё сразу), число одновременных браузеров и нужно ли сбросить стенд перед прогоном.

Всё выполняется на раннере GitHub: там поднимается приложение в Docker, ставится Python и Chrome, тесты идут в headless-режиме, отчёт публикуется автоматически. На вашем компьютере не запускается ничего.
...
Результат виден там же: статус шагов, короткая сводка на странице прогона, архив с результатами и [Allure-отчёт](https://dilligan1.github.io/ui-automation-framework/). Если тест упал, в отчёте рядом с шагом лежит скриншот экрана.
